...
if action == "Create New Chart":
    ...
    if chart_html:
        ...
        if chart_data:
            ...
            st.write("Data Preview:")
            source_data = st.experimental_data_editor(source_data)
            measurement_unit = chart_data.get("data", {}).get(list(chart_data["data"].keys())[0], {}).values()[0].split(' ')[-1]
            y_axis_label = chart_data.get("data", {}).get(list(chart_data["data"].keys())[0], {}).keys()[0].split(' ')[0] + ' (Mbps)'
...
if source_data is not None:
    ...
    value_columns = st.multiselect("Select the columns for tests:", source_data.columns[1:], default=source_data.columns[1:])
    ...
    # Select chart size
    chart_size = st.selectbox("Select the chart size:", ["Small", "Full Width"], index=1 if chart_width == 805 else 0)
    if chart_size == "Small":
        chart_width = 500
        chart_height = 300
    else:
    ...
    grouping_method = st.selectbox("Group data by:", ["Provider", "Test Type"], index=0 if grouping_method == "Provider" else 1)
    ...
    if st.button("Generate HTML"):
        ...
        if grouping_method == "Provider":
            ...
            for i, col in enumerate(mapped_columns.values()):
                values = [
                    float(value.split(' ')[0]) if pd.notna(value) else null_value
